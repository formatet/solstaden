<script>
  ...
  const CAT_LABEL = { bar: 'Bar', cafe: 'Café', restaurant: 'Restaurang' };
  ...
  $effect(() => { selectedTime; category; clearTimeout(fetchTimeout); fetchTimeout = setTimeout(fetchPlaces, 250); });
...
<div class="sky">
  ...
  <div class="controls">
    ...
    <div class="filter-row">
      <select bind:value={category} class="cat-select">
...
<style>
  ...
  .hero h1 {
    ...
    font-size:2.2rem;
    color:#fff;
    text-shadow:0 2px 12px #0285b766;
    letter-spacing:-.5px;
    line-height:1;
  }
  .tagline {
  ...
  .search {
    ...
    box-shadow:0 2px 10px #00558822;
  }
  .search:focus { outline:3px solid #f5c518; }
  .filter-row { display:flex; gap:0.4rem; align-items:center; }
  .cat-select {
    flex:1; border:none; border-radius:999px; padding:0.45rem 0.9rem;
    font-size:0.85rem;
    background:rgba(255,255,255,0.85);
    box-shadow:0 1px 6px #00558822;
  }
  ...
  .sort-btns button.active {
    ...
    color:#7a5000;
    font-weight:700;
    box-shadow:0 2px 8px #f5c51855;
  }
  .sort-btns button:hover:not(.active) { background:rgba(255,255,255,0.92); }

  .stats {
    max-width:600px; margin:0.25rem auto; width:100%;
    padding:0 0.75rem;
  ...
  main {
    flex:1; max-width:600px; margin:0.5rem auto; width:100%;
  ...
  .card {
    ...
    box-shadow:0 3px 12px #0055881a;
    cursor:pointer;
  ...
  .suggest-btn {
    background:#f5c518; color:#7a5000; font-weight:700;
    border-radius:999px; padding:0.55rem 1.4rem;
    text-decoration:none; font-size:0.9rem;
    box-shadow:0 3px 10px #f5c51844;
